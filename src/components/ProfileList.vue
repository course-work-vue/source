...
<template>
  <div class="col col-xs-9 col-lg-12 mt-4 list">
    ...
    <div class="mb-3 col col-12">
      ...
      <table v-if="loading" class="table">
        <tbody>
        <tr v-for="n in profilesPerPage" :key="n">
    ...
    <table v-else class="table">
        ...
        <tbody>
          ...
          <tr v-for="profile in paginatedProfiles" :key="profile.prof_id">
            <td>{{ profile.prof_id }}</td>
            <td>{{ profile.prof_name }}</td>
            <td>{{ profile.dir_name }}</td>
            <td>{{ profile.dir_code }}</td>
        
            <td>
              <!-- кнопошка -->
              <button @click="viewProfileDetail(profile.prof_id)" class="btn btn-primary btn-sm"><i class="material-icons-outlined">visibility</i>Детали</button>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        profiles: [], // массив всех студентов
        currentPage: 1, // теущий номер страницы
        profilesPerPage: 10, // кол-во студентов на странице
        maxPageButtons: 3,
        loading: true,
        searchQuery: ''
      };
  ...
  totalPages() {
    const filteredProfiles = this.filterProfiles();
    return Math.ceil(filteredProfiles.length / this.profilesPerPage);
  },
  paginatedProfiles() {
  ...
  pageRange() {
    ...
    return this.profiles.filter(profile => {
      return (
        String(profile.prof_name).toLowerCase().includes(queryString) ||
        String(profile.dir_name).toLowerCase().includes(queryString) ||
        String(profile.dir_code).includes(queryString)
      );
    });
  },
      // грузим данные
  ...
  this.$router.replace({ query });
      ...
      viewRightsDetail(){
        this.$router.push(`/Rights/profiles`);
      },
