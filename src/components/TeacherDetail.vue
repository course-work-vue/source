        ...
        <Form  @submit="updateTeacher" :validation-schema="schema" v-slot="{ errors }">
          ...
          <div>
            ...
            <div class="form-group  mt-3">
              ...
              <router-link to="/teachers" class="mx-2 btn btn-secondary  float-start">Отмена</router-link>
            </div>
            <div class="form-group float-end">
              <button class="btn btn-danger float-end" @click="deleteTeacher">
      ...
      methods: {
        ...
        async loadTeacherDetail() {
          ...
          try {
            ...
            this.editedTeacher = { ...response.data };
          } catch (error) {
        ...
        async updateTeacher() {
          try {
            ...
            const response = await UserService.updateTeacherById(this.teacher.teacher_id, this.editedTeacher.first_name, this.editedTeacher.last_name, this.editedTeacher.patronymic);
            response.data;
            this.teacher = { ...this.editedTeacher };
            this.loading=false;
            this.toast.success("Успешно обновили преподавателя!");
          } catch (error) {
        ...
        async deleteTeacher() {
          try {
            ...
            const response = await UserService.deleteTeacherById(this.teacher.teacher_id);
            response.data;
            this.teacher = { ...this.editedTeacher };
            this.loading=false;
            this.toast.success("Успешно удалили преподавателя!");
          } catch (error) {
            console.error('Ошибка загрузки данных о преподавателе:', error);
          }
        },
        async loadTegrsusData() {
          try {
            const response = await UserService.getTegrsusAsIdText(); 
