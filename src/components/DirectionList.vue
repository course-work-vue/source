...
<template>
  <div class="col col-xs-9 col-lg-12 mt-4 list">
    ...
    <div class="mb-3 col col-12">
      <button @click="navigateToAddDirection" class="btn btn-primary float-start" type="button"><i class="material-icons-outlined">add</i>Добавить направление</button>
    ...
    <table v-else class="table">
        ...
        <tbody>
          ...
          <tr v-for="direction in paginatedDirections" :key="direction.dir_id">
            <td>{{ direction.dir_id }}</td>
            <td>{{ direction.dir_name }}</td>
            <td>{{ direction.dir_code }}</td>
           
            <td>
              <!-- кнопошка -->
              <button @click="viewDirectionDetail(direction.dir_id)" class="btn btn-primary btn-sm"><i class="material-icons-outlined">visibility</i>Детали</button>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        directions: [], // массив всех студентов
        currentPage: 1, // теущий номер страницы
        directionsPerPage: 10, // кол-во студентов на странице
        maxPageButtons: 3,
        loading: true,
        searchQuery: ''
      };
  ...
  totalPages() {
    const filteredDirections = this.filterDirections();
    return Math.ceil(filteredDirections.length / this.directionsPerPage);
  },
  paginatedDirections() {
    const startIndex = (this.currentPage - 1) * this.directionsPerPage;
    const endIndex = startIndex + this.directionsPerPage;

    const filteredDirections = this.filterDirections();
    return filteredDirections.slice(startIndex, endIndex);
  },
  pageRange() {
    ...
    return this.directions.filter(directions => {
      return (
        String(directions.dir_name).toLowerCase().includes(queryString) ||
        String(directions.dir_code).toLowerCase().includes(queryString)
      );
    });
  },
      // грузим данные
  ...
  this.$router.replace({ query });
      ...
      viewDirectionDetail(directionId) {
        this.$router.push(`/directions/${directionId}`);
      },
      viewRightsDetail(){
        this.$router.push(`/Rights/directions`);
      },

      navigateToAddDirection() {
    
        this.$router.push(`/addDirection`); // Navigate to the AddStudent route
    },
    },
