...
<template>
  <div class="col col-xs-9 col-lg-12 mt-4 list">
    ...
    <div class="mb-3 col col-12">
      <button @click="navigateToAddCw" class="btn btn-primary float-start" type="button"><i class="material-icons-outlined">add</i>Добавить студента</button>
      ...
      <table v-if="loading" class="table">
        <tbody>
        <tr v-for="n in cwsPerPage" :key="n">
    ...
    <table v-else class="table">
        ...
        <tbody>
          ...
          <tr v-for="cw in paginatedCws" :key="cw.cw_id">
            <td>{{ cw.course_work_id }}</td>
            <td>{{ cw.course_work_teacher_id }}</td>
            <td>{{ cw.course_work_theme }}</td>
            <td>{{ cw.full_name }}</td>
            <td>{{ cw.course_work_kafedra }}</td>
            <td>
              <!-- кнопошка -->
              <button @click="viewCwDetail(cw.cw_id)" class="btn btn-primary btn-sm"><i class="material-icons-outlined">visibility</i>Детали</button>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        cws: [], // массив всех студентов
        currentPage: 1, // теущий номер страницы
        cwsPerPage: 10, // кол-во студентов на странице
        maxPageButtons: 3,
        loading: true,
        searchQuery: ''
      };
  ...
  totalPages() {
    const filteredCws = this.filterCws();
    return Math.ceil(filteredCws.length / this.cwsPerPage);
  },
  paginatedCws() {
    const startIndex = (this.currentPage - 1) * this.cwsPerPage;
    const endIndex = startIndex + this.cwsPerPage;

    const filteredCws = this.filterCws();
    return filteredCws.slice(startIndex, endIndex);
  },
  pageRange() {
    ...
    return this.cws.filter(cw => {
      return (
        String(cw.full_name).toLowerCase().includes(queryString) ||
        String(cw.group_name).toLowerCase().includes(queryString) ||
        String(cw.formatted_date_of_birth).includes(queryString) ||
        String(cw.enrollment_order).includes(queryString)
      );
    });
  },
      // грузим данные
      async loadCwsData() {
        try {
          ...
          this.cws = Array.isArray(response.data) ? response.data : [response.data];
          this.loading=false;
        } catch (error) {
          console.error('Error loading cws data:', error);
        }
      },
  ...
  this.$router.replace({ query });
      ...
      viewCwDetail(cwId) {
        this.$router.push(`/cws/${cwId}`);
      },
      viewRightsDetail(){
        this.$router.push(`/Rights/cws`);
      },

      navigateToAddCw() {
    
        this.$router.push(`/addCw`); // Navigate to the AddCw route
    },
    },
